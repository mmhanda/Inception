...
wp core install --url=${DOMAIN} --title=${DOMAIN} --admin_user=${MYSQL_USER} --admin_password=${MYSQL_PASSWORD} --admin_email=${ADMIN_EMAIL} --allow-root
wp user create ${ADMIN_USER} ${ADMIN_EMAIL_TWO} --role=administrator --user_pass=${MYSQL_PASSWORD} --allow-root
mkdir /run/php && chown root:root /run/php && chmod 777 /run/php
